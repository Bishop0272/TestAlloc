    ...
    if os.path.isfile(credentialsPath):
        ...
        for i, row in enumerate(data):
            ...
            if not is_blank(sign_up_date) and is_blank(email_sign_up):
                ...
                worksheet.update_cell(row_number, 14, ct_email)  # Update 'CT-Email (Sign Up)'

            if not is_blank(payment_15_date) and not is_blank(
                    sign_up_date) and payment_15_date == sign_up_date and is_blank(email_15):
                ct_email = generate_ct_email()
                worksheet.update_cell(row_number, 15, ct_email)  # Update 'CT-Email (15%)'

            if not is_blank(payment_35_date) and not is_blank(
                    payment_15_date) and payment_35_date == payment_15_date and is_blank(email_35):
                ct_email = generate_ct_email()
                worksheet.update_cell(row_number, 16, ct_email)  # Update 'CT-Email (35%)'

            if not is_blank(payment_40_date) and is_blank(email_40):
                ct_email = generate_ct_email()
                worksheet.update_cell(row_number, 17, ct_email)  # Update 'CT-Email (40%)'

            if not is_blank(zero_snags_date) and is_blank(email_zero_snags):
                ct_email = generate_ct_email()
                worksheet.update_cell(row_number, 18, ct_email)  # Update 'CT-Email (ZeroSnags)'

            if not is_blank(eoj_date) and is_blank(email_eoj):
                ct_email = generate_ct_email()
                worksheet.update_cell(row_number, 19, ct_email)  # Update 'CT-Email (EOJ)'
